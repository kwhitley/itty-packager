...
async function getCommitMessage(newVersion, silent = false) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Show placeholder with cursor positioned at start
    const placeholderText = '\x1b[90mpress enter to skip\x1b[0m'
    process.stdout.write(`💬 Commit message: ${placeholderText}`)
    // Move cursor back to start of placeholder
    process.stdout.write(`\x1b[${placeholderText.length - 8}D`) // -8 for ANSI codes
    
    let inputLines = []
    let firstInput = true
    let placeholderCleared = false

    // Handle raw input for immediate placeholder clearing
    process.stdin.setRawMode(true)
    const originalDataHandler = process.stdin.listeners('data')[0]
    
    const clearPlaceholder = () => {
      if (!placeholderCleared) {
        // Clear entire line and rewrite prompt
        process.stdout.write('\r\x1b[K💬 Commit message: ')
    ...
    const dataHandler = (chunk) => {
      ...
      // Handle Escape key
      if (key === '\x1b') {
        process.stdout.write('\r\x1b[K💬 Commit message: cancelled\n')
        rl.close()
        reject(new Error('User cancelled with Escape key'))
        return
      }
      
      // Clear placeholder on any printable character or backspace
      if (!placeholderCleared && (key.length === 1 || key === '\x7f')) {
        clearPlaceholder()
      }
      
      // Pass through to readline
      if (originalDataHandler) {
        originalDataHandler.call(process.stdin, chunk)
      }
    }
    ...
    rl.on('close', () => {
      ...
      process.stdin.removeAllListeners('data')
      if (originalDataHandler) {
        process.stdin.on('data', originalDataHandler)
      }
      process.stdin.setRawMode(false)
