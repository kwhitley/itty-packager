  ...
  try {
    ...
    if (!dryRun) {
      ...
      try {
        ...
        console.log(`⚠️  npm session expired — logging in...`)
    ...
    if (!noGit && !dryRun && (shouldPush || shouldTag)) {
      try {
        ...
        if (error.message.includes('cancelled')) {
          console.log('❌ Release cancelled - reverting version and exiting')
    ...
    if (!noGit && !dryRun && (shouldPush || shouldTag)) {
      ...
      if (shouldTag) {
        ...
        await runCommand(`git tag -f -a v${newVersion} -m "${commitMessage}"`, rootPath, verbose)
      }
